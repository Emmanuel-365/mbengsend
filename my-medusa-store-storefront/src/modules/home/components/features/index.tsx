...
export default function Features() {
    return (
        <div className="py-24 bg-brand-dark text-white overflow-hidden relative">
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-brand-primary/20 rounded-full blur-[100px] pointer-events-none" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
                    {features.map((feature, index) => (
                        <div key={index} className="flex flex-col items-start gap-4">
                            <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-2xl mb-2 text-brand-primary border border-white/5 shadow-[0_0_15px_rgba(0,102,204,0.3)]">
                                {feature.icon}
                            </div>
                            <div>
                                <h3 className="text-xl font-display font-bold mb-2 tracking-wide">
                                    {feature.title}
                                </h3>
